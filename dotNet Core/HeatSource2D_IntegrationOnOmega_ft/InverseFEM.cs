...
namespace HeatSource2D_IntegrationOnOmega_ft {
    class InverseFEM {
        ...
        public double[] CG(double[,] node, int[,] elem, int[] dirichlet, int Nx, int Ny, int Nt, double T,
            ...
            String path_infor = Path.Combine(Environment.CurrentDirectory, @"..\Results\Integration_ft_" + Nx + "_" + eps * 100 + "%_log1.txt");
            String path_fh = Path.Combine(Environment.CurrentDirectory, @"..\Results\Integration_ft_" + Nx + "_" + eps * 100 + "%_1.txt");
