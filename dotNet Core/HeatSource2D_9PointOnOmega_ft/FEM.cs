...
namespace HeatSource2D_9PointOnOmega_ft {
    class FEM {
        public double[] SolveFEM(double[,] node, int[,] elem, int[] dirichlet, int Nx, int Ny, double jacobi,
            ...
            double[] f0, Func<double, double, double, double> q0, double[] f1, Func<double, double, double, double> q1, double[] f2, Func<double, double, double, double> q2,
            double[] f3, Func<double, double, double, double> q3, double[] f4, Func<double, double, double, double> q4, double[] f5, Func<double, double, double, double> q5,
            double[] f6, Func<double, double, double, double> q6, double[] f7, Func<double, double, double, double> q7, double[] f8, Func<double, double, double, double> q8) {
            ...
            LinearEquation(A, ref b, node, elem, Nx, Ny, jacobi, axx, ayy, dxu0, dyu0, f0, q0, f1, q1, f2, q2, f3, q3, f4, q4, f5, q5, f6, q6, f7, q7, f8, q8);
        ...
        public void LinearEquation(Dictionary<Tuple<int, int>, double> A, ref double[] b, double[,] node, int[,] elem, int Nx, int Ny, double jacobi,
           ...
           double[] f6, Func<double, double, double, double> q6, double[] f7, Func<double, double, double, double> q7, double[] f8, Func<double, double, double, double> q8) {
            ...
            for (int noe = 0; noe < NoE; noe++) {
                ...
                double[] ff0 = new double[] { f0[elem[noe, 0] / ((Nx + 1) * (Ny + 1))], f0[elem[noe, 1] / ((Nx + 1) * (Ny + 1))],
                                              f0[elem[noe, 2] / ((Nx + 1) * (Ny + 1))], f0[elem[noe, 3] / ((Nx + 1) * (Ny + 1))]};
                double[] ff1 = new double[] { f1[elem[noe, 0] / ((Nx + 1) * (Ny + 1))], f1[elem[noe, 1] / ((Nx + 1) * (Ny + 1))],
                                              f1[elem[noe, 2] / ((Nx + 1) * (Ny + 1))], f1[elem[noe, 3] / ((Nx + 1) * (Ny + 1))]};
                double[] ff2 = new double[] { f2[elem[noe, 0] / ((Nx + 1) * (Ny + 1))], f2[elem[noe, 1] / ((Nx + 1) * (Ny + 1))],
                                              f2[elem[noe, 2] / ((Nx + 1) * (Ny + 1))], f2[elem[noe, 3] / ((Nx + 1) * (Ny + 1))]};
                double[] ff3 = new double[] { f3[elem[noe, 0] / ((Nx + 1) * (Ny + 1))], f3[elem[noe, 1] / ((Nx + 1) * (Ny + 1))],
                                              f3[elem[noe, 2] / ((Nx + 1) * (Ny + 1))], f3[elem[noe, 3] / ((Nx + 1) * (Ny + 1))]};
                double[] ff4 = new double[] { f4[elem[noe, 0] / ((Nx + 1) * (Ny + 1))], f4[elem[noe, 1] / ((Nx + 1) * (Ny + 1))],
                                              f4[elem[noe, 2] / ((Nx + 1) * (Ny + 1))], f4[elem[noe, 3] / ((Nx + 1) * (Ny + 1))]};
                double[] ff5 = new double[] { f5[elem[noe, 0] / ((Nx + 1) * (Ny + 1))], f5[elem[noe, 1] / ((Nx + 1) * (Ny + 1))],
                                              f5[elem[noe, 2] / ((Nx + 1) * (Ny + 1))], f5[elem[noe, 3] / ((Nx + 1) * (Ny + 1))]};
                double[] ff6 = new double[] { f6[elem[noe, 0] / ((Nx + 1) * (Ny + 1))], f6[elem[noe, 1] / ((Nx + 1) * (Ny + 1))],
                                              f6[elem[noe, 2] / ((Nx + 1) * (Ny + 1))], f6[elem[noe, 3] / ((Nx + 1) * (Ny + 1))]};
                double[] ff7 = new double[] { f7[elem[noe, 0] / ((Nx + 1) * (Ny + 1))], f7[elem[noe, 1] / ((Nx + 1) * (Ny + 1))],
                                              f7[elem[noe, 2] / ((Nx + 1) * (Ny + 1))], f7[elem[noe, 3] / ((Nx + 1) * (Ny + 1))]};
                double[] ff8 = new double[] { f8[elem[noe, 0] / ((Nx + 1) * (Ny + 1))], f8[elem[noe, 1] / ((Nx + 1) * (Ny + 1))],
                                              f8[elem[noe, 2] / ((Nx + 1) * (Ny + 1))], f8[elem[noe, 3] / ((Nx + 1) * (Ny + 1))]};

                double[,] loc = local(x, y, t, jacobi, axx, ayy, dxu0, dyu0, ff0, q0, ff1, q1, ff2, q2, ff3, q3, ff4, q4, ff5, q5, ff6, q6, ff7, q7, ff8, q8);
                for (int i = 0; i < 4; i++) {
        ...
        public double[,] local(double[] x, double[] y, double[] t, double jacobi,
           ...
           double[] f6, Func<double, double, double, double> q6, double[] f7, Func<double, double, double, double> q7, double[] f8, Func<double, double, double, double> q8) {
            ...
            double fq_hat(double xi, double eta, double zeta) =>
                f0[0] * q0(x[0], y[0], t[0]) * (1 - xi - eta - zeta) + f0[1] * q0(x[1], y[1], t[1]) * xi + f0[2] * q0(x[2], y[2], t[2]) * eta + f0[3] * q0(x[3], y[3], t[3]) * zeta +
                f1[0] * q1(x[0], y[0], t[0]) * (1 - xi - eta - zeta) + f1[1] * q1(x[1], y[1], t[1]) * xi + f1[2] * q1(x[2], y[2], t[2]) * eta + f1[3] * q1(x[3], y[3], t[3]) * zeta +
                f2[0] * q2(x[0], y[0], t[0]) * (1 - xi - eta - zeta) + f2[1] * q2(x[1], y[1], t[1]) * xi + f2[2] * q2(x[2], y[2], t[2]) * eta + f2[3] * q2(x[3], y[3], t[3]) * zeta +
                f3[0] * q3(x[0], y[0], t[0]) * (1 - xi - eta - zeta) + f3[1] * q3(x[1], y[1], t[1]) * xi + f3[2] * q3(x[2], y[2], t[2]) * eta + f3[3] * q3(x[3], y[3], t[3]) * zeta +
                f4[0] * q4(x[0], y[0], t[0]) * (1 - xi - eta - zeta) + f4[1] * q4(x[1], y[1], t[1]) * xi + f4[2] * q4(x[2], y[2], t[2]) * eta + f4[3] * q4(x[3], y[3], t[3]) * zeta +
                f5[0] * q5(x[0], y[0], t[0]) * (1 - xi - eta - zeta) + f5[1] * q5(x[1], y[1], t[1]) * xi + f5[2] * q5(x[2], y[2], t[2]) * eta + f5[3] * q5(x[3], y[3], t[3]) * zeta +
                f6[0] * q6(x[0], y[0], t[0]) * (1 - xi - eta - zeta) + f6[1] * q6(x[1], y[1], t[1]) * xi + f6[2] * q6(x[2], y[2], t[2]) * eta + f6[3] * q6(x[3], y[3], t[3]) * zeta +
                f7[0] * q7(x[0], y[0], t[0]) * (1 - xi - eta - zeta) + f7[1] * q7(x[1], y[1], t[1]) * xi + f7[2] * q7(x[2], y[2], t[2]) * eta + f7[3] * q7(x[3], y[3], t[3]) * zeta +
                f8[0] * q8(x[0], y[0], t[0]) * (1 - xi - eta - zeta) + f8[1] * q8(x[1], y[1], t[1]) * xi + f8[2] * q8(x[2], y[2], t[2]) * eta + f8[3] * q8(x[3], y[3], t[3]) * zeta;
