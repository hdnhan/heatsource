...
namespace SpaceTime2D {
    class FEM {
        public double[] SolveFEM(double[,] node, int[,] elem, int[] dirichlet, double jacobi,
            ...
            LinearEquation(A, ref b, node, elem, jacobi, axx, ayy, dxu0, dyu0, F);
        ...
        public void LinearEquation(Dictionary<Tuple<int, int>, double> A, ref double[] b, double[,] node, int[,] elem, double jacobi,
           Func<double, double, double, double> axx, Func<double, double, double, double> ayy,
            ...
            for (int noe = 0; noe < NoE; noe++) {
                ...
                double[,] loc = local(x, y, t, jacobi, axx, ayy, dxu0, dyu0, F);
                for (int i = 0; i < 4; i++) {
