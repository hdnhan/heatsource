...
namespace HeatSource1D_Q_fxt
{
    class FEM
    {
        public double[] SolveFEM(double[,] node, int[,] elem, int[] dirichlet, double jacobi,
            Func<double, double, double> a, Func<double, double> dxu0, double[] f, Func<double, double, double> q)
        {
            ...
            LinearEquation(A, ref b, node, elem, jacobi, a, dxu0, f, q);
            ...
            Func<double, double, double> a, Func<double, double> dxu0, double[] f, Func<double, double, double> q)
        {
            ...
            for (int noe = 0; noe < NoE; noe++)
            {
                ...
                double[] ff = new double[] { f[elem[noe, 0]], f[elem[noe, 1]], f[elem[noe, 2]] };
                double[,] loc = local(x, t, jacobi, a, dxu0, ff, q);
                for (int i = 0; i < 3; i++)
                {
            ...
            Func<double, double, double> a, Func<double, double> dxu0, double[] f, Func<double, double, double> q)
        {
            ...
            Func<double, double, double> fq_hat = (xi, tau)
                => f[0] * q(x[0], t[0]) * (1 - xi - tau) + f[1] * q(x[1], t[1]) * xi + f[2] * q(x[2], t[2]) * tau;

            Func<double, double, double> fq_hat1 = (xi, tau)
                => fq_hat(xi, tau) * (1 - xi - tau) - sgn * a_hat(xi, tau) * dxu0_hat(xi, tau) * grad_phi[0, 0] / jacobi;
            Func<double, double, double> fq_hat2 = (xi, tau)
                => fq_hat(xi, tau) * xi - sgn * a_hat(xi, tau) * dxu0_hat(xi, tau) * grad_phi[1, 0] / jacobi;
            Func<double, double, double> fq_hat3 = (xi, tau)
                => fq_hat(xi, tau) * tau - sgn * a_hat(xi, tau) * dxu0_hat(xi, tau) * grad_phi[2, 0] / jacobi;

            res[3, 0] = slvs.Gauss2D(fq_hat1) * jacobi;
            res[3, 1] = slvs.Gauss2D(fq_hat2) * jacobi;
            res[3, 2] = slvs.Gauss2D(fq_hat3) * jacobi;
            return res;
        }
